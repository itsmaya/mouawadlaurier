...
/* source_lang est OMIS quand l'appelant ne le fournit pas : DeepL détecte
   alors la langue de départ et la renvoie dans detected_source_language.
   L'envoyer à null faisait échouer la détection. */
if (!empty($body['source_lang'])) {
    $payload['source_lang'] = strtoupper($body['source_lang']);
}

/* tag_handling=xml : le générateur envoie le gras et le souligné sous forme
   de balises <b> / <u>, pour que DeepL les repositionne autour du mot traduit
   au lieu de les déplacer ou de les perdre. */
if (!empty($body['tag_handling'])) {
    $payload['tag_handling'] = $body['tag_handling'];
}
